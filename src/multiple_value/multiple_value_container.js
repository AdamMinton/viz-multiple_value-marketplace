        ...
        if(measures_like.find(m => m.name === measure.name) && pivots.length > 0 ){
          pivots.forEach(function(pivot,pivotIndex) {
            ...
            rows.push({
              ...
              valueFormat: measure.value_format.replaceAll("\\", '') ?? "#,##0",
          ...
          rows.push({
            ...
            valueFormat: measure.value_format ?? "#,##0".replaceAll('\\', ''),
            formattedValue: rowLen === index ? formatSpreadsheet(measure.value_format ?? "#,##0".replaceAll('\\', ''),row[measure.name].value) : LookerCharts.Utils.textForCell(row[measure.name]),
            row_number: index,
